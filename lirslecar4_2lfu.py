...
class lirslecar4_2lfu(page_replacement_algorithm):
    ...
    def __init__(self, cache_size, learning_rate=0, initial_weight=0.5, discount_rate=1, visualize=0):
        ...
        self.LIRS = lirs2_1.Lirs2_1(self.N, 0.07)
        ...
        self.LIRS.pageFreq = self.pageFreq
        self.LIRS.History_1 = self.Hist1
    ...
    ##########################################
    def addToCache(self, page, poly, page_temp_freq):
        ...
        self.LIRS.request(page)
        self.LFUalg.request(page)
        self.LFUalg.PQ.increase(page, page_temp_freq-1)
        self.pageFreq[page] = page_temp_freq
    ...
    def addToseparateHistory(self, poly, cacheevict):
        histevict = None
        if poly == -1:
            if cacheevict not in self.LIRS.prunedHIR:
                del self.LIRS.prunedHIR[cacheevict]

            if cacheevict not in self.LIRS.hirStack:
                del self.LIRS.hirStack[cacheevict]
                del self.LIRS.lirStack[cacheevict]
            del self.pageFreq[cacheevict]
            ...
            if self.Hist1.size() == self.H:
                ...
                if histevict is not None:
                    ...
                    if (histevict not in self.LIRS.hirStack) and (histevict not in self.LIRS.prunedHIR):
                        del self.pageFreq[histevict]
            self.Hist1.add(cacheevict)
            #self.info['lru_count'] += 1
        # elif (poly == 1):
        #     if self.LIRS.nonresidentHIRsInStack + len(self.LIRS.prunedHIR) >= self.H:
        ...
        if (cacheevict not in self.Hist1) and (cacheevict not in self.LIRS.hirStack) and (cacheevict not in self.LIRS.prunedHIR):
            del self.pageFreq[cacheevict]
        ...
        #assert len(self.pageFreq) <= 3*self.N
    ...
    def request(self, page):
        ...
        if page in self.LFUalg:
            ...
            if page in self.Hist1:
            ...
            elif page in self.LIRS.hirStack or (page in self.LIRS.prunedHIR):
                assert page not in self.LIRS
                ...
                assert page in self.pageFreq
                self.pageFreq[page] += 1
                if page in self.LIRS.prunedHIR:
                    del self.LIRS.prunedHIR[page]

                #self.Hist2.delete(page)
                reward[1] = -1
            else:
                #assert page not in self.pageFreq, "<%d>" % page
                self.pageFreq[page] = 1
                self.page_fault_count += 1
            page_temp_freq =  self.pageFreq[page]
            #################
            ## Update Weights
            #################
            ...
            self.addToCache(page, poly, page_temp_freq)
