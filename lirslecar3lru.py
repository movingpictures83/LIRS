...
class lirslecar3lru(page_replacement_algorithm):
    ...
    def pageHitUpdate(self, page):
        ...
        assert page in self.LRUalg

        self.LIRS.request(page)
        self.LRUalg.request(page)
    ...
    def addToCache(self, page):
        ...
        assert page not in self.LRUalg
    ...
    def evictPage(self, pg, poly):
        ...
        assert pg in self.LRUalg
        assert pg in self.LIRS

        self.LRUalg.disk.delete(pg)
    ...
    def request(self, page):
        ...
        if page in self.LRUalg:
            ...
            ## Remove from Cache
            ####################
            if self.LRUalg.disk.size() == self.N:
                ################
                ## Choose Policy
                ################
