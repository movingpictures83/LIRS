...
class lirslecar4lirs(page_replacement_algorithm):
    ...
    def __init__(self, cache_size, learning_rate=0, initial_weight=0.5, discount_rate=1, visualize=0):
        ...
        self.LIRS = lirs2.Lirs2(self.N, 0.01)
        self.LIRS2 = lirs2.Lirs2(self.N, 0.99)
    ...
    def pageHitUpdate(self, page):
        ...
        assert page in self.LIRS2

        self.LIRS.request(page)
        self.LIRS2.request(page)

        # if self.log:
        #     self.LIRS.f3.write("Page HIT\n")
        #     self.LIRS.f3.write("____________________________________________________\n")


    ##########################################
    ## Add a page to cache using policy 'poly'
    ##########################################
    def addToCache(self, page, poly):
        assert page not in self.LIRS
        assert page not in self.LIRS2
        ...
        #     self.LIRS.f3.write("_________________________________________________________n")
    ...
    def selectEvictPage(self, policy):
        r = next(iter(self.LIRS2.residentHIRList))
    ...
    def request(self, page):
        ...
        if page in self.LIRS2:
            ...
            # Remove from Cache
            poly = -2
            if self.LIRS2.currentHIRSSize + self.LIRS2.currentLIRSSize == self.N:
                # Choose Policy
