...
class CacheMetaData(object):
    def __init__(self):
        ...
        super(CacheMetaData, self).__setattr__("prunned", False)
...
class lirs4_2_1(page_replacement_algorithm):
    f3 = open('debugFile', 'w+')

    def __init__(self, cache_size, hirsRatio=0.01):

        self.size = cache_size#int(param['cache_size'])
        if self.size < 4:
             self.size = 4
    ...
    def __contains__(self, page):
        if page in self.lirStack and not self.lirStack[page].prunned:
            return self.lirStack[page].isResident
        elif page in self.residentHIRList and not self.residentHIRList[page].prunned:
            return True
        else:
            return False
    ...
    def __getitem__(self, page):
        if page in self.lirStack:
            return self.lirStack[page]
        elif page in self.residentHIRList:
            return self.residentHIRList[page]
        else:
            raise "Key Error - Not Found %d" % page



    def request(self, page, from_lfu_hist=False, saved_freq=0):
        ...
        #assert len(self.lirStack) <= self.M*self.size
        ...
        if from_lfu_hist:
            ...
            self.pageFault = True
            self.addToCache(page, saved_freq)
        else:
            if page in self.lirStack and not self.lirStack[page].prunned:
    ...
    def limitStackSize(self):
        pruneSize = self.currentHIRSSize + self.currentLIRSSize + self.nonresidentHIRsInStack - self.size * self.M
        ...
        if poly == 1 or poly == -1:
            if page in self.lirStack and not self.lirStack[page].prunned:
                if self.lirStack[page].isLir:
                    ...
                    return self.deleteLIRpage(page)
                else:
                    Lirs2.f3.write("delete hir in lirs\n")
                    return self.deleteHIRInLIRStack(page)
            elif page in self.residentHIRList and not self.residentHIRList[page].prunned:
                Lirs2.f3.write("delete red\n")
                return self.deleteResidentHIR(page)
            else:
                raise KeyError("Page not in cache")
        else:
            # freq = -1
            # if page in self.residentHIRList:
            #     freq = self.residentHIRList[page].freq
            #     del self.residentHIRList[page]
            # if page in self.lirStack:
            #     if freq != -1:
            #         assert freq == self.lirStack[page].freq
            #     else:
            #         freq = self.lirStack[page].freq
            #     del self.lirStack[page]
            # if page in self.hirStack:
            #     if freq != -1:
            #         assert freq == self.hirStack[page].freq
            #     else:
            #         freq = self.hirStack[page].freq
            #     del self.hirStack[page]
            # assert freq != -1
            # assert freq != 0
            # return freq
            if page in self.lirStack and not self.lirStack[page].prunned:
                if self.lirStack[page].isLir:
                    Lirs2.f3.write("delete lir by LFU\n")
                    return self.deleteLIRpageByLFU(page)
                else:
                    Lirs2.f3.write("delete hir in lirs by LFU\n")
                    return self.deleteHIRInLIRStackByLFU(page)
            elif page in self.residentHIRList and not self.residentHIRList[page].prunned:
                Lirs2.f3.write("delete red by lfu\n")
                return self.deleteResidentHIRByLFU(page)
            else:
                raise KeyError("Page not in cache LFU")
    ...
    def deleteLIRpage(self, page):
        ...
        return freq
    def deleteLIRpageByLFU(self, page):
        freq = self.lirStack[page].freq
        del self.lirStack[page]
    ...
    def deleteHIRInLIRStackByLFU(self, page):
        data = self.lirStack[page]
        freq = data.freq
        result = data.isResident
        del self.lirStack[page]
    ...
    def deleteResidentHIRByLFU(self, page):
        freq = self.residentHIRList[page].freq
        del self.residentHIRList[page]
        if page in self.lirStack:
            tmpData = self.lirStack[page]
    ...
    def print_stack(self):
        Lirs5 = self
        Lirs5.f3.write("LIR stack:\n")
        for key in self.lirStack:
            data = self.lirStack[key]
            Lirs5.f3.write("LIR <%d> red: <%r> lir: <%r> prunned<%r>\n" % (key, data.isResident, data.isLir, data.prunned))

        Lirs5.f3.write("\n\nHIR stack:\n")
        for key in self.hirStack:
            data = self.hirStack[key]
            Lirs5.f3.write("HIR <%d> red: <%r> lir: <%r> prunned<%r>\n" % (key, data.isResident, data.isLir, data.prunned))

        Lirs5.f3.write("\n\nresidentHIRList stack:\n")
        for key in self.residentHIRList:
            data = self.residentHIRList[key]
            Lirs5.f3.write("redHIR <%d> red: <%r> lir: <%r> prunned<%r>\n" % (key, data.isResident, data.isLir, data.prunned))
        Lirs5.f3.write("\n\n")
