...
class Lirs2_1(page_replacement_algorithm):
    ...
    def request(self, page):
        ...
        assert len(self.lirStack) <= 2*self.size + 1
        assert len(self.prunedHIR) + self.nonresidentHIRsInStack <= self.size
    ...
    def keepNmetadata(self):
        pruneSize = self.nonresidentHIRsInStack + len(self.prunedHIR) - self.size
