...
class CacheMetaData(object):
    def __init__(self):
        ...
        super(CacheMetaData, self).__setattr__("type", -1)
...
class Lirs5(page_replacement_algorithm):
    f3 = open('debugFile', 'w+')
    ...
    def delete(self, page, poly):
        ...
        if page in self.lirStack:
            if self.lirStack[page].isLir:
                ...
                self.deleteLIRpage(page, poly)
            else:
                #Lirs2.f3.write("delete hir in lirs\n")
                self.deleteHIRInLIRStack(page, poly)
        elif page in self.residentHIRList:
            #Lirs2.f3.write("delete red\n")
            self.deleteResidentHIR(page, poly)
        else:
            raise KeyError("Page not in cache")
        #Lirs2.f3.write("print stack: \n")
        #self.print_stack()

    def deleteLIRpage(self, page, poly):
        data = self.lirStack[page]
    ...
    def deleteHIRInLIRStack(self, page, poly):
        data = self.lirStack[page]
        result = data.isResident
    ...
    def deleteResidentHIR(self, page, poly):
        del self.residentHIRList[page]
        if page in self.lirStack:
