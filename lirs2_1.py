...
class Lirs2_1(page_replacement_algorithm):
    f3 = open('debugFile', 'w+')
    ...
    def request(self, page):
        ...
        assert len(self.lirStack) <= 1.5*self.size + 1
        assert len(self.prunedHIR) + self.nonresidentHIRsInStack <= 0.5* self.size
    ...
    def limitStackSize(self):
        pruneSize = self.currentHIRSSize + self.currentLIRSSize + self.nonresidentHIRsInStack - self.size * 1.5
    ...
    def pruneStack(self):
        ...
        for key in self.lirStack:
            ...
            if key in self.hirStack:
                if not self.hirStack[key].isResident:
                    self.prunedHIR[key] = data
    ...
    def keepNmetadata(self):
        pruneSize = self.nonresidentHIRsInStack + len(self.prunedHIR) - self.size*0.5

        pruneKeys = []
        for key in self.prunedHIR:
            if pruneSize <= 0:
                break
            pruneKeys.append(key)
            #del self.prunedHIR[key]
            #if key not in self.History_1:
            #del self.pageFreq[key]
            pruneSize -= 1
    ...
    def print_stack(self):
        Lirs2_1.f3.write("LIR stack:")
        for key in self.lirStack:
            data = self.lirStack[key]
            Lirs2_1.f3.write("LIR <%d> red: <%r> lir: <%r>\n" % (key, data.isResident, data.isLir))

        Lirs2_1.f3.write("\n\nHIR stack:")
        for key in self.hirStack:
            data = self.hirStack[key]
            Lirs2_1.f3.write("HIR <%d> red: <%r> lir: <%r>\n" % (key, data.isResident, data.isLir))

        Lirs2_1.f3.write("\n\nresidentHIRList stack:")
        for key in self.residentHIRList:
            data = self.residentHIRList[key]
            Lirs2_1.f3.write("redHIR <%d> red: <%r> lir: <%r>\n" % (key, data.isResident, data.isLir))
        Lirs2_1.f3.write("\n\n")

        Lirs2_1.f3.write("\n\nprunnedStack stack:")
        for key in self.prunedHIR:
            data = self.prunedHIR[key]
            Lirs2_1.f3.write("prunedHIR <%d> red: <%r> lir: <%r>\n" % (key, data.isResident, data.isLir))
        Lirs2_1.f3.write("\n\n")
