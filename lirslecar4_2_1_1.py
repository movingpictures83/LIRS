...
class lirslecar4_2_1_1(page_replacement_algorithm):
    def __init__(self, cache_size, learning_rate=0, initial_weight=0.5, discount_rate=1, visualize=0):
        ...
        self.LIRS = lirs4_2_1_1_adaptive.lirs4_2_1_1(self.N, 0.07)
    ...
    def addToCache(self, page, saved_freq, from_lfu_hist):
        ...
        self.LIRS.addToCache(page, from_lfu_hist, saved_freq)
    ...
    def evictPage(self, pg, lfu_deletion, avoid_removal):
        if self.log:
        ...
        freq2 = self.LIRS.delete(pg, lfu_deletion, avoid_removal)
    ...
    def request(self, page):
        if self.log:
            ...
            for v in self.LIRS.SStack:
                if self.LIRS.SStack[v].isResident:
                    assert v in self.LFUalg, "%d" % v
            for v in self.LIRS.QStack:
        ...
        if page in self.LFUalg:
            ...
            if page in self.Hist1:
            ...
            elif page in self.LIRS.nonresidentHIRsInS:
                data2 = self.LIRS.nonresidentHIRsInS[page]
                #if data2.isPrunned:
                #    assert page not in self.LIRS.SStack
                #    assert data2.isResident == False
                #    assert data2.isLir == False
                #    assert data2.freq > 0
                #    assert page not in self.LIRS.QStack
                #
                #    del self.LIRS.nonresidentHIRsInS[page]
                #else:
                assert page in self.LIRS.SStack
                data = self.LIRS.SStack[page]
                ...
                self.page_fault_count = 0
                pageevict = page
                saved_freq = data2.freq

                #self.Hist2.delete(page)
                reward[1] = -1
            else:
                assert page not in self.LIRS.SStack
                assert page not in self.LIRS.nonresidentHIRsInS
                assert page not in self.LIRS.QStack
                saved_freq = 0
            ...
            if self.LFUalg.PQ.size() == self.N: #cache is full
                assert self.LIRS.numberHIRpages + self.LIRS.numberLIRpages == self.N
                ################
                ## Choose Policy
                ################
                ...
                #################
                lfu_removal = (poly == 0)
                evicteds_freq = self.evictPage(cacheevict, lfu_removal, page)
                self.unique_block_count += 1
